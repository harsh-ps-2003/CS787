...
# Stable Diffusion fine-tuning pipeline for medical image generation
# Features:
...
def log_validation(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, epoch):
    ...
    for i in range(len(args.validation_prompts)):
        with torch.autocast("cuda"):
            ...
            if not os.path.exists(os.path.join(args.output_dir, 'validation', args.validation_prompts[i])):
                os.mkdir(os.path.join(args.output_dir, 'validation', args.validation_prompts[i]))
            image.save(os.path.join(args.output_dir, 'validation', args.validation_prompts[i], f'epoch-{epoch}.png'))
        images.append(image)
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            np_images = np.stack([np.asarray(img) for img in images])
...
def main():
    ...
    # Check both HF_HUB_OFFLINE and TRANSFORMERS_OFFLINE (both can enable offline mode)
    hf_offline = os.environ.get("HF_HUB_OFFLINE", "0") == "1"
    transformers_offline = os.environ.get("TRANSFORMERS_OFFLINE", "0") == "1"
    use_local_files = hf_offline or transformers_offline
    
    # Warn if offline mode is set but we need to download models
    if use_local_files and accelerator.is_main_process:
        ...
        if transformers_offline:
            logger.warning("TRANSFORMERS_OFFLINE=1 is set. This may also enable offline mode.")
        logger.warning("If models are not cached, unset these environment variables for the first run.")
    ...
    if args.use_custom_unet:
        raise ValueError(
            "Custom MedicalUNet is no longer available. "
    ...
    if args.use_ema:
        ...
        # Apply attention gates to EMA UNet if enabled
        if args.use_skip_attention_gates:
            add_attention_gates_to_unet(ema_unet, verbose=False)
